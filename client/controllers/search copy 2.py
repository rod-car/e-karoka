...
def search(query_terms, query_vector, start):
    doc_ids_list = []
    ...
    if doc_ids_list:
        ...
        if common_doc_ids:
            tfidf_records = TfidfValues.objects.filter(document_id__in=common_doc_ids).values_list('document_id', 'tfidf_vectors')
            tfidf_vectors = [json.loads(record[1]) for record in tfidf_records]
