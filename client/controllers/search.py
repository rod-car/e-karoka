...
def search(query_terms, query_vector):
    tfidf_records = TfidfValues.objects.all().values_list('document_id', 'tfidf_vectors')
    document_ids = [record[0] for record in tfidf_records]
    ...
    doc_similarity_pairs = list(zip(document_ids, similarities))
